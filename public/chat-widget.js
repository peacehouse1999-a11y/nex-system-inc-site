...
(function () {
  ...
  const FAQ = [
    {
      ...
      answer: "Nous avons deux pôles principaux : le <strong>Référencement Local &amp; Ads</strong> (visibilité sur Google Maps, publicité ciblée) et les <strong>Formations / Coaching</strong> aux outils IA et créatifs. Nous avons aussi un <strong>Programme Artistes</strong> dédié.",
    },
    {
      keywords: ['prix', 'tarif', 'coût', 'cout', 'combien'],
      answer: "Nos tarifs dépendent de vos besoins et objectifs. Le plus simple est de nous laisser un message — je peux vous aider à le faire tout de suite si vous voulez.",
    },
    {
    ...
    {
      keywords: ['artiste', 'peintre', 'photographe', 'musicien', 'artisan'],
      answer: "Notre <strong>Programme Artistes</strong> s'adresse aux peintres, photographes, musiciens et artisans d'art : un marketing pensé pour valoriser votre démarche, sans jamais toucher à votre création.",
    },
    {
    ...
    {
      keywords: ['ads', 'publicité', 'publicite', 'annonce'],
      answer: "Nous gérons des campagnes de publicité payante ciblée pour amener rapidement des clients qualifiés vers votre entreprise.",
    },
  ];
  ...
  const QUICK_REPLIES_MAIN = [
    ...
    { label: 'Programme Artistes', value: 'artistes' },
  ...
  function handleUserInput(rawText, displayText) {
    ...
    if (state.mode === 'contact-telephone') {
      ...
      return botSay('Quel est votre besoin principal ?', [
        ...
        { label: 'Publicité (Ads)', value: 'ads' },
        { label: 'Formation / Coaching', value: 'formation' },
        { label: 'Programme Artistes', value: 'artiste' },
        { label: 'Autre', value: 'autre' },
      ]);
    }
    if (state.mode === 'contact-besoin') {
      const key = ['seo', 'ads', 'formation', 'artiste'].includes(text) ? text : 'autre';
    ...
    if (text === 'services' || text === 'formations' || text === 'artistes') {
      const answer = findFaqAnswer(text === 'artistes' ? 'artiste' : text);
